...
dfc_1440M = pickle.load(open(savepath+"df_1440M_CPC_FINAL.p","rb"))

for i in np.arange(0,np.shape(dfc_1440M)[1]):
    cur_values = copy.deepcopy(dfc_1440M.iloc[:,i].values)
    idxs = []
    ...
    dfc_1440M.iloc[:,i] = copy.deepcopy(new_cur_values)
...
df_1440M_DJF = pd.concat([df_1440M_D,df_1440M_JF])

# Group seasonal data by year
dfs = ['1440M']; yr_num = mo_num - 1
yys = np.arange(2007,2018)
for i in np.arange(0,len(dfs)):
    for j in yys: 
        exec('df_'+dfs[i]+'_'+str(j)+' = dfc_'+dfs[i]+'[(dfc_'+dfs[i]+'.iloc[:,'+str(yr_num)+'] == '+str(j)+')]')
...
##############################################################################
##############################################################################
##### For Longer Term Cycles
casenames = ['1440M','1440M_MAM','1440M_JJA','1440M_SON','1440M_DJF']; 
varnames = ['cpc_avg']
cnames_plt = ['MAM','JJA','SON','DJF']
dt = 24 # hours
num_days = 28
overlap = 0.5
days_conv = 1 #convert hourly data to daily data
...
for v in np.arange(0,len(varnames)):
    ...
    addsave = 'x28day_xxxxPUB_'+varname+'_'+str(num_days)+'day_'+str(dt*60)+'M_'+str(overlap)+'overlap_ylim'
...
#        for yyyy in np.arange(2009,2014):   
        for yyyy in np.arange(2007,2017):   
...
#            #CPC
            axs[k].text(2.15,0.165,texts[k]+' ('+str(np.count_nonzero(~np.isnan(pwr_s_comb[k][:,0])))+')',fontsize = 18)
...
#                axs[k].set_xlabel('Period (Days)')
        ...
        if cur_case.__contains__('1440'):
            ...
            for i in np.arange(1,5):            
                ...
                #
                import pickle
                pickle.dump((tp,tp_data,tp_4ddata,tp_pwr,tnp,tnp_data,tnp_4ddata,tnp_pwr),open(savepath+"times_power_"+cnames_plt[cnt_plt]+"_"+varname+"_28day.p","wb"))
                cnt_plt = cnt_plt + 1
