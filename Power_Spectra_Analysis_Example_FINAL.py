...
# Need to first run power spectral analysis to get the FFT, IFFT data
# Plot in publication should be id_peiod = 4, i = 13

per_hrs = pr_dys*24


id_period = 4 # 0 = ALL, 4 = DJF
pr_desired = 0.5 #days
...
#    ax[0].plot(TIMES[id_pwr[i]],temp_sum,'-c',lw=6,label='0-14 Frequencies')
    ...
    ax.legend()
    ax.xaxis.set_ticks([datetime.datetime(2012,2,23,0) + datetime.timedelta(hours=int(i*12)) for i in np.arange(0,15)])

    ax.xaxis.set_major_formatter(mdates.DateFormatter("%d|%H"))

    ax.set_xlabel('UTC Time (Feb. 2012 Day|Hour)')
    ax.set_ylabel('N$_{7-30nm}$ Concentration Anomaly \n ($cm^{-3}$)')
#    ax.set_title(str(TIMES[id_pwr[i]][0])[0:10])
    ax.grid()
    plt.tight_layout()
    plt.show()
    plt.savefig('/Volumes/GoogleDrive/My Drive/Research_Projects/CCN_Closure_M.S./Paper/Final_Submission/Reviews/Final_Accepted/xPUB_Example_12+24hour_Cycle.png')
    plt.savefig('/Volumes/GoogleDrive/My Drive/Research_Projects/CCN_Closure_M.S./Paper/Final_Submission/Reviews/Final_Accepted/xPUB_Example_12+24hour_Cycle.eps')
#
#
#
#
